...
class starter {
	public static void main(String args[]) throws FileNotFoundException {
		...
		String TEMP2=textFile;
		// Uses the fillArray method that has been built below. This fills the array 'arr' with each word individually in the text file.
		System.out.println();
		...
		*/
		while(true){
			...
			if(choice.equals("file")){
				System.out.println("The file is called: "+textFile);
			}
				...
				int NumPrint=sc.nextInt();
				printsNumber(arr, NumPrint);
			}
			else if(choice.equals("change")){
				System.out.println("What new text would you like to swap to?: ");
				String NewFile = sc.nextLine();
				TEMP2=NewFile;
			}
			else if(choice.equals("check")){
				System.out.println("What word do you want to check for? ");
				String CheckWord = sc.nextLine();
				System.out.println("The word you entered has "+checksArray(arr, CheckWord)+" duplicates");
			}
			else if(choice.equals("common")){
				System.out.println("The most common word in the selected Text File is: "+mostCommon(arr));
			}
			else{
	...
	*/
	public static void printsNumber(String[] arr, int num){
		int TEMP1=num;
		if(TEMP1<arr.length){
			System.out.println("Printing "+TEMP1+" Words of the Script");
			for(int i=0; i<TEMP1; i++){
				System.out.print(arr[i]+" ");
			}
		}
		else if(TEMP1>arr.length){
	...
	*/
	public static int checksArray(String[] arr, String check){
		...
		int DUPE1=0;
		for(int i=0; i<arr.length; i++){
			if(arr[i].equals(TEMP3)){
				DUPE1=DUPE1+1;
			}
		}
		return DUPE1;	// Dummy value, delete when needed.
	}
